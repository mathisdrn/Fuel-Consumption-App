...
metrics = df.group_by('release_year').agg([
...
temp = df.group_by('release_year').agg([
    pl.col('make').count().alias('model_count')
])
...
temp = df.group_by(['release_year', 'vehicle_class']).agg([
    pl.col('make').count().alias('model_count')
]).with_columns([
    (pl.col('model_count') / pl.col('model_count').sum().over('release_year')).alias('proportion')
])
...
temp = df.group_by(['release_year', 'fuel_type']).agg([
    pl.col('make').count().alias('model_count')
]).with_columns([
    (pl.col('model_count') / pl.col('model_count').sum().over('release_year')).alias('proportion')
])
...
temp = df.group_by(['vehicle_class', 'fuel_type']).agg([
    pl.col('make').count().alias('model_count')
]).with_columns([
    (pl.col('model_count') / pl.col('model_count').sum().over('vehicle_class')).alias('proportion')
])

# Order vehicle classes by model count
order = df.group_by('vehicle_class').agg([
    pl.col('make').count().alias('model_count')
]).sort('model_count', descending=True)['vehicle_class'].to_list()
...
temp = df.filter(pl.col('fuel_type') == fuel_type).group_by(['release_year', 'vehicle_class']).agg([
    pl.col('emissions').mean().alias('emissions')
])
...
temp = df.filter(pl.col('vehicle_class') == vehicle_class).group_by(['release_year', 'fuel_type']).agg([
    pl.col('emissions').mean().alias('emissions')
])
